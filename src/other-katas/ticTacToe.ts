...
type WinState = `X won by row` | 'X won by column' | 'X won by diagonal' | `O won by row` | 'O won by column' | 'O won by diagonal' | 'There was a draw' | 'The game has not been finished yet';
type Player = 'x' | 'o' | '';
...
const calcWinState = (BoardState: Player [][]): WinState => {
    ...
    const xwonByColumn = BoardState.some((row, index) => row[index] === 'x'); //BoardState.every(row => row[0] === 'x') || BoardState.every(row => row[1] === 'x') || BoardState.every(row => row[2] === 'x');
    ...
    const ywonByColumn = BoardState.some((row, index) => row[index] === 'o'); //BoardState.every(row => row[0] === 'o') || BoardState.every(row => row[1] === 'o') || BoardState.every(row => row[2] === 'o');
    const ywonByDiagonal = BoardState[0][0] === 'o' && BoardState[1][1] === 'o' && BoardState[2][2] === 'o'|| BoardState [0][2] === 'o' && BoardState[1][1] === 'o' && BoardState[2][0] === 'o';
...
console.log(calcWinState([['x','','x'],['x',"",'o'],["x","",""]]))